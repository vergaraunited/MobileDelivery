                    ...
                    using (var command = new SqlCommand(SP.ToString(), cnn))
                    {
                        ...
                        switch (dat.command)
                        {
                            ...
                            case eCommand.CheckManifest:
                                ...
                                Logger.Info($"QueryData Manifest. Cmd:{retVal.Command.ToString()}  reqId: {retVal.RequestId.ToString()}  ");
                                dat.command = eCommand.ManifestLoadComplete;
                                break;
                            case eCommand.Trucks:
                                req = (manifestRequest)dat;
                            ...
                            case eCommand.GenerateManifest:
                                ...
                                retVal = new ManifestMasterData((manifestMaster)dat, 0);// { LINK= ((manifestMaster)dat).LINK, RequestId = new Guid(dat.requestId) };
                                break;
                            default:
                                {
